...
train = pd.read_csv("../data/kaggle_house_pred/kaggle_house_pred_train.csv")
test = pd.read_csv("../data/kaggle_house_pred/kaggle_house_pred_test.csv")
...
def k_fold_cross_valid(X_train, y_train):
    assert k > 1
...
train_loss, test_loss = k_fold_cross_valid(X_train, y_train)
print("%d-fold validation: Avg train loss: %f, Avg test loss: %f" % (k, train_loss, test_loss))

def learn(X_train, y_train, test):
    net = get_net()
